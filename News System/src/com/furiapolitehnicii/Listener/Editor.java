...
public abstract class Editor implements Listener, Runnable {
	private String name;
	private ConcurentQueue<Event> events;

	public Editor(String name, ConcurentQueue<Event> events) {
		this.name = name;
		this.events = events;
	}
	...
	public void dispatch(Event event) {
		System.out.println(
				"Hei " + name + ", " + event.getIntro() + System.lineSeparator()
						+ event.getNews().getTitle() + System.lineSeparator());
	}
	...
	private static boolean isNewsDeleted(News news) {
		if (news.isDeleted()) {
			System.out.println(
					"The news was erased from the editorial. Cannot publish/update/delete a deleted news"
							+ System.lineSeparator() + news.getTitle());
			return true;
		}
		return false;
	}
